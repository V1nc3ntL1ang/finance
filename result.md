...
调参只使用 Valid。每个模型先在 Train 上训练，然后在 Valid 上从 107352 个仓位候选里选择参数，最后只在 Test 上报告结果。
...
当前最好的可交易策略是 `hist_gradient_boosting + 显式20特征 + rank_linear`。它在同一 Test 区间内比 buy_hold 多 10.88 个百分点收益，同时最大回撤从 -17.26% 降到 -5.85%，夏普从 2.18 提高到 3.83。
